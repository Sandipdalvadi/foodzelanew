...
class UsersController extends Controller
{
    ...
    public function list(Request $request)
    {
        ...
        $totalData = User::where('role',4)->where('is_deleted',0);
        if($status == 3){
        ...
        {            
            ...
            $posts =  User::where('role',4)->where('is_deleted',0);
                if($status != 3){
            ...
            $totalFiltered = User::where('role',4)->where('is_deleted',0);
            if($status != 3){
        ...
        if($posts)
        {
            foreach ($posts as $post) 
            {
                ...
                $image = '<img style="width:50px;height:50px;" class="b-r-10" src="'.file_exists_in_folder('profile_pic', $img).'"alt="" />';
                ...
                $data['status'] = "<select class='js-example-basic-single form-control' onchange=changeStatus(this,'".route('admin.users.changeStatus',['id'=>$post->id])."')>
                ...
                <a class='btn btn-primary' href=".route('admin.users.detail',['id'=>$post->id])."><i class='icofont icofont-eye-alt'></i></a>
                <form style='float:left;margin-left:6px;' method='POST' action=".route('admin.users.delete',['id'=>$post->id]).">";
    ...
    {   
        ...
        $user->is_deleted = 1;
        $user->save();
        return redirect()->route('admin.users.index')->with('message',"Users Deleted Successfully"); 
    }
